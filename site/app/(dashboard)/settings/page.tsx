...
import { useGoogleStatus, useDisconnectGoogleAccount } from "@/hooks/useGoogle"
import { apiClient } from "@/lib/api-client"
import type { GoogleConnectedAccount } from "@/lib/api-client"
...
function GoogleAccountRow({ account }: { account: GoogleConnectedAccount }) {
  const { mutate: disconnect, isPending } = useDisconnectGoogleAccount()

  return (
    <div className="flex items-center justify-between p-3 rounded-xl border border-slate-100 bg-slate-50">
      <div className="flex items-center gap-3 min-w-0">
        <div className="h-8 w-8 rounded-full flex items-center justify-center shrink-0" style={{ background: "rgba(16,185,129,0.1)" }}>
        ...
        <div className="min-w-0">
          ...
          {account.label && account.label !== account.googleEmail && (
            <p className="text-xs text-slate-400 truncate">{account.googleEmail}</p>
          )}
          {account.listings.length > 0 && (
            <p className="text-[11px] text-blue-500 mt-0.5">
              {account.listings.length} fiche{account.listings.length > 1 ? "s" : ""} liée{account.listings.length > 1 ? "s" : ""}
            </p>
          )}
        </div>
      </div>
      <Button
        ...
        disabled={isPending}
        className="text-red-600 border-red-200 hover:bg-red-50 shrink-0 ml-3"
      >
        {isPending ? (
    ...
    <Card>
      <CardHeader>
        <div className="flex items-center gap-2">
          ...
          <div>
            <CardTitle>Comptes Google</CardTitle>
            <CardDescription>Connectez plusieurs comptes Google pour gérer vos clients</CardDescription>
          </div>
        </div>
      </CardHeader>
      <CardContent className="space-y-4">
